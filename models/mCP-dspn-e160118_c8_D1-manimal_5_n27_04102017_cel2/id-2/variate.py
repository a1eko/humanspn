...
def bap_ca(id2dist, morphofile, version=0):
    ...
    print('bAP Ca 60-100', np.mean( [sumCa[i] for i in index if dist[i] > 60 and dist[i] < 100]))
...
def main(args):
    ...
    for morph in args.morphology:
        ...
        for par, param_values in enumerate(models):
            ...
            figs.append(fig)
            feature_values = efel.get_feature_values(traces, features_sp)
            n = len(responses)
            for i, resp in enumerate(responses):
                if feature_values[i]['Spikecount'] == 0:
                    ...
                    trace['stim_end'] = [stim_spec['stimuli'][0]['delay'] +
                                         stim_spec['stimuli'][0]['duration']]
                    feature_values[i] = efel.get_feature_values([trace], features_sp + features_iv)[0]
                    ...
                    trace['stim_end'] = [stim_spec['stimuli'][0]['delay'] +
                                         stim_spec['stimuli'][0]['duration']]
                    feature_values[i] = efel.get_feature_values([trace], features_sp + features_ap)[0]
                else:
                    trace = {}
                    trace['T'] = responses[resp]['time']
                    trace['V'] = responses[resp]['voltage']
                    ...
                    trace['stim_end'] = [stim_spec['stimuli'][0]['delay'] +
                                         stim_spec['stimuli'][0]['duration']]
                    feature_values[i] = efel.get_feature_values([trace], features_sp + features_ap + features_id)[0]
