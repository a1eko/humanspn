...
stim_spec = {
    ...
    'stimuli': [
        {
            'amp_range': [-0.650, 0.700],
            'delay': 700.0,
            'duration': 2000.0,
            'totduration': 3000.0
         }, {
            'amp_range': [0.0009, 0.0009],
            'delay': 0.0,
            'duration': 3000.0,
            'totduration': 3000.0
         }
...
def shorten(name, length):
    if len(name) > length:
        short_name = name[: int(length / 2)] + "..." + name[-int(length / 2) :]
    else:
        short_name = name
    return short_name
...
def bap_ca(id2dist, morphofile, version=0):
    ...
    print('bAP Ca 30-40 ', sumCa[norm_ind])
    print('bAP Ca 40-50 ', np.mean( [sumCa[i] for i in index if dist[i] > 40 and dist[i] < 50]))
    print('bAP Ca 50-60 ', np.mean( [sumCa[i] for i in index if dist[i] > 50 and dist[i] < 60]))
    print('bAP Ca 60-70 ', np.mean( [sumCa[i] for i in index if dist[i] > 60 and dist[i] < 70]))
    print('bAP Ca 60-100*1.7', np.mean( [sumCa[i] for i in index if dist[i] > 60*1.7 and dist[i] < 100*1.7]))

    #CforMean    = [sumCa[i] / sumCa[norm_ind]  for i in index if dist[i] > 60 and dist[i] < 100]
    CforMean    = [sumCa[i] / sumCa[norm_ind]  for i in index if dist[i] > 60*1.7 and dist[i] < 100*1.7]
    ...
    for morph in args.morphology:
        ...
        for par, param_values in enumerate(models):
            ...
            for resp in responses:
                ...
                traces.append(trace)
            fig, ax = plt.subplots()
            title = f"{cell_id}, {os.path.splitext(os.path.basename(morph))[0]}, {par=}"
            fig.suptitle(shorten(title, 40), fontsize=14)
            resps = list(responses.keys())[::-1]
            nresp = len(resps)
            for i, resp in enumerate(resps, start=1):
                trace = {}
                trace["T"] = responses[resp]["time"]
                trace["V"] = responses[resp]["voltage"]
                trace["stim_start"] = [stim_spec["stimuli"][0]["delay"]]
                trace["stim_end"] = [
                    stim_spec["stimuli"][0]["delay"]
                    + stim_spec["stimuli"][0]["duration"]
                ]
                alpha = 1 / (nresp / 4 - i + 1) if i <= int(nresp / 4) else 1.0
                ax.plot(trace["T"], trace["V"], color="tab:blue", lw=1, alpha=alpha)
